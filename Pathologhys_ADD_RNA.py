...
class Mob(pygame.sprite.Sprite):  # ENEMIGO
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        ...
        self.speedx = 0 #random.randrange(1,3)                                      
...
class Explosion(pygame.sprite.Sprite): # ANIMACION DE EXPLOSION
    def __init__(self, center, size):
        ...
        self.rect.center = center                                # es para que la animacion se ubique en el centro de la imagen que se va a sustituir
        self.frame = 0                                           # para saber en que frame se esta inicialmente esta en el cero 0
...
def go_ventana4():
    ...
    running = True
    while running:
        global tiempo_pulsar_botón, t, percepcion
        reloj.tick(FPS)
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
...
def basedatosmundo1():
    global mundo, nivel, numvidas, porcentaje, velocidad_enem_o_medic, percepcion, predic1, dspc

    conexion.execute("insert into datos(mundo,nivel,vidasrestantes,porcentajevida,dificultadnivel,percepciondificultad,prediccion,ppm) values (?,?,?,?,?,?,?,?)", (mundo,nivel,numvidas,porcentaje,velocidad_enem_o_medic,percepcion,predic1,dspc))
    conexion.commit()
...
# Funcion - Modelo Red Neuronal Artificial
def timer_interruptmundo1():

    global predic1, prediccion1, porcentaje, dspc, numvidas, mundo
    
    # cargar json y crear el modelo
    json_file = open('modelojuego.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    loaded_model = keras.models.model_from_json(loaded_model_json)

    # cargar pesos al nuevo modelo
    loaded_model.load_weights("modelojuego.h5")
    print("Cargado modelo desde disco.")
    
    # Compilar modelo cargado y listo para usar.
    loaded_model.compile(loss='sparse_categorical_crossentropy', optimizer=keras.optimizers.RMSprop(), metrics=['accuracy'])  
    # Observacion: El modelo quedo lamacenado en 'loaded_model'

    # Probando el modelo
    if mundo == 1:
        testing_data = np.array([[porcentaje, dspc, numvidas]], "float64")  
        prediccion1 = loaded_model.predict(testing_data).round()

        if prediccion1[:,3]==1:
            prediccion1=3
            predic1=3
        elif prediccion1[:,5]==1:
            prediccion1=5
            predic1=5
...
while running:
    ...
    if mundo ==1 and t:
        ...
        if tiempo_actual - tiempo_pulsar_botón > 60000 and nivel == 1: 
            ...
            # Condiciones de ajuste automatico de parametros - Basadas en la prediccion que establece la Red Neuronal Artificial 
            if prediccion1 == 3 and cont == 0:
                velocidad_enem_o_medic = velocidad_enem_o_medic
                cont += 1

            elif prediccion1 == 3 and cont == 1 and velocidad_enem_o_medic < 6:
                velocidad_enem_o_medic += 0.5
                cont -= 1
            
            elif prediccion1 == 5 and velocidad_enem_o_medic > 4 and cont == 0:
                velocidad_enem_o_medic -= 0.5
            
            elif prediccion1 == 5 and velocidad_enem_o_medic > 4 and cont == 1:
                velocidad_enem_o_medic -= 0.5
                cont -= 1
            
            elif prediccion1 == 3 and cont == 0 and velocidad_enem_o_medic == 6:
                velocidad_enem_o_medic = velocidad_enem_o_medic
